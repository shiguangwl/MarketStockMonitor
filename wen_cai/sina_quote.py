...
SINA_API_URL = "https://hq.sinajs.cn/"
...
US_NAME_IDX = 0
US_PRICE_IDX = 1
US_DATETIME_TZ_IDX = 25  # 例如: "Jul 21 05:15PM EDT"
US_YEAR_IDX = 29         # 例如: "2025"
...
def _parse_hk_stock(data_parts: List[str]) -> Optional[SinaPriceDataPoint]:
    """解析港股 (rt_hk) 数据。"""
    if len(data_parts) <= max(HK_DATE_IDX, HK_TIME_IDX):
        return None

    try:
        name = data_parts[HK_NAME_IDX]
        price = _to_float(data_parts[HK_PRICE_IDX])
        date_str = data_parts[HK_DATE_IDX].replace("/", "-")
        time_str = data_parts[HK_TIME_IDX]
        
        datetime_str = f"{date_str} {time_str}"
        # 港股直接赋予时区信息
        naive_dt = datetime.strptime(datetime_str, "%Y-%m-%d %H:%M:%S")
        timestamp = BEIJING_TZ.localize(naive_dt)
...
def _parse_us_stock(data_parts: List[str]) -> Optional[SinaPriceDataPoint]:
    """
    解析美股 (gb) 数据。
    """
    if len(data_parts) <= max(US_DATETIME_TZ_IDX, US_YEAR_IDX):
        return None

    try:
        name = data_parts[US_NAME_IDX]
        price = _to_float(data_parts[US_PRICE_IDX])
        
        year_str = data_parts[US_YEAR_IDX]
        datetime_with_tz_str = data_parts[US_DATETIME_TZ_IDX]
        ...
        source_tz = US_TZ_MAP.get(tz_abbr)
        if not source_tz:
        ...
        beijing_dt = source_aware_dt.astimezone(BEIJING_TZ)
...
def fetch_sina_quotes(codes: List[str]) -> Dict[str, SinaPriceDataPoint]:
    """
    从新浪财经获取指定代码列表的实时行情。
    ...
    list_str = ",".join(codes)
    url = f"{SINA_API_URL}?rn={timestamp}&list={list_str}"
    ...
    try:
        ...
        for code_from_api, data_str in matches:
            ...
            if code_from_api.startswith('rt_hk'):
                point = _parse_hk_stock(data_parts)
            elif code_from_api.startswith('gb_'):
                point = _parse_us_stock(data_parts)
            else:
...
def get_hsi_quote() -> Optional[SinaPriceDataPoint]:
    """获取恒生指数的实时报价。"""
    result = fetch_sina_quotes(['rt_hkHSI'])
    return result.get('rt_hkHSI')

def get_nasdaq_quote() -> Optional[SinaPriceDataPoint]:
    """获取纳斯达克综合指数的实时报价。"""
    result = fetch_sina_quotes(['gb_ixic'])
...
if __name__ == "__main__":
    ...
    quotes = fetch_sina_quotes(stock_codes_to_fetch)
